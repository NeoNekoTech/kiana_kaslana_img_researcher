...
# Définition d'un bloc de convolution de base avec BatchNorm et ReLU
...
# Définition d'un module Inception pour extraire des caractéristiques complexes
class Inception(nn.Module):
    def __init__(self):
        super(Inception, self).__init__()
        # Différents chemins pour extraire des caractéristiques
...
# Définition de la classe FaceBoxes pour la détection de visages
class FaceBoxes(nn.Module):
    def __init__(self, phase, size, num_classes):
        ...
        # Initialisation des couches pour la régression des boîtes et la classification
    ...
    def forward(self, x):
        ...
        if self.phase == "test":
            output = (loc.view(loc.size(0), -1, 4),
                      F.softmax(conf.view(-1, self.num_classes), dim=-1))
        else:
            output = (loc.view(loc.size(0), -1, 4),
